...
class SystemTestCase(StaticLiveServerTestCase):
    ...
    def test_admin(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/admin')

        self.selenium.find_element_by_id('id_username')

    def test_about(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/about')

        body = self.selenium.find_element_by_tag_name('body')
        self.assertIn('Sander Maijers', body.text)

    def test_all_centres(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/all_centres')

        table = self.selenium.find_element_by_id('all_centres')
        table.find_element_by_tag_name('tr')

    def test_centre(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/centre/1')
    ...
    def test_centres_contacts(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/centres_contacts')

        table = self.selenium.find_element_by_id('centres_contacts')
        table.find_element_by_tag_name('tr')

    def test_consortia(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/consortia')

        table = self.selenium.find_element_by_id('consortia')
        table.find_element_by_tag_name('tr')

    def test_contacting(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/contacting')

        body = self.selenium.find_element_by_tag_name('body')
        self.assertIn('Contacting', body.text)

    def test_contact(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/contact/1')

        table = self.selenium.find_element_by_id('contact')
        self.assertIn('ePPN', table.text)

    def test_fcs(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/fcs')

        body = self.selenium.find_element_by_tag_name('body')
        self.assertIn('FCS endpoint', body.text)

    def test_map(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/map')

        body = self.selenium.find_element_by_tag_name('body')
        self.assertIn('geographical overview', body.text)

    def test_oai_pmh(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/oai_pmh')

        table = self.selenium.find_element_by_id('oai-pmh_endpoints')
        table.find_element_by_tag_name('tr')

    def test_spf(self):
        self.selenium.get(self.live_server_url.replace('localhost', '127.0.0.1') + '/spf')
