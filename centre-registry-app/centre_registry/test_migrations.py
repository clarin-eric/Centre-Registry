...
class MigrationChainTest(TransactionTestCase):
    ...
    def test_migration_chain(self):
        ...
        init_state = migrator.apply_initial_migration(targets=("centre_registry", os.path.splitext(os.path.basename(migrations[0]))[0]))

        # roll forward to final state
        final_state = migrator.apply_tested_migration(targets=("centre_registry", os.path.splitext(os.path.basename(migrations[-1]))[0]))
        migrator.reset()